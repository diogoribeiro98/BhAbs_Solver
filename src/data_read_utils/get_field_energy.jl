...
function simple_integration_2D(x_vec,y_vec,z_vec)
    
    sum_vector = zeros(length(y_vec))

    for idx in 1:length(sum_vector)
        sum_vector[idx] = simple_integration(x_vec,z_vec[:,idx])
    end

    sum = simple_integration(y_vec , sum_vector)
...
function get_field_energy( fname::String, every::Int , tlimit::Float64)
   ...
   # Vpot = potential_matrix(p)
    ...
    for i in 0:every:max_plot_iter
        ...
        E = simple_integration_2D(x , y ,Energy_matrix)
